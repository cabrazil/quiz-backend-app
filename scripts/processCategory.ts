...
async function fetchCategoryId(categoryName: string) {
  try {
...
async function processCategory(categoryName: string) {
  try {
    console.log(`\nIniciando processamento da categoria: ${categoryName}`)
    ...
    // Limpar perguntas existentes da categoria
    await prisma.question.deleteMany({
      where: { category: categoryName }
    })
    console.log(`Perguntas existentes da categoria ${categoryName} removidas`)
    ...
    for (const question of questions) {
      console.log('\nProcessando pergunta:', question.question)

      // Decodificar base64
      const decodedText = decodeBase64(question.question)
      const decodedCorrectAnswer = decodeBase64(question.correct_answer)
      const decodedOptions = question.incorrect_answers.map(opt => decodeBase64(opt))
    ...
    console.log(`\nProcessamento da categoria ${categoryName} concluído com sucesso!`)
  } catch (error) {
...
// Processar a categoria Entertainment: Video Games
processCategory('Entertainment: Video Games') 